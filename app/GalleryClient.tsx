...
export default function GalleryClient({
  ...
  const leftColumn = templates.filter((_, i) => i % 2 === 0);
  const rightColumn = templates.filter((_, i) => i % 2 === 1);
        ...
        <section className="flex flex-col gap-[18px] px-7 pb-9">
          ...
          >
            ...
            <motion.div className="flex flex-col gap-3" variants={stagger}>
              {leftColumn.map((t, i) => (
                <TemplateCard key={t.slug} template={t} index={i * 2} />
              ))}
            </motion.div>
            {/* Right column */}
            <motion.div className="flex flex-col gap-3" variants={stagger}>
              {rightColumn.map((t, i) => (
                <TemplateCard key={t.slug} template={t} index={i * 2 + 1} />
              ))}
            </motion.div>
          </motion.div>
        </section>
