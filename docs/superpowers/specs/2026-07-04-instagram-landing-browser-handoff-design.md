...
Tapping "Abrir no navegador" immediately dismisses the panel while attempting
the native browser handoff. If the device blocks the handoff, the visitor can
continue using the landing page inside Instagram. Reloading the page shows the
prompt again; dismissal is intentionally not persisted.

## Failure Handling

- Unsupported or changed Instagram user agents fail open and show the normal
  landing page.
- Failed external-browser handoffs leave the landing page available after the
  visitor taps the primary action.
