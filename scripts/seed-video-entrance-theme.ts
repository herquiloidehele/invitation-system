/**
 * Seed script for the Video-Entrance invitation theme.
 *
 * Creates (or updates) one theme row with `layout: "video-entrance"` so the
 * renderer at `app/[slug]/InvitationView.tsx` routes invitations using this
 * theme through the new `<VideoEntrancePage>` flow (single tap-to-play video
 * as cover + hero → timed text reveal → Canva iframe → inline RSVP — no
 * envelope).
 *
 * Run: npx tsx scripts/seed-video-entrance-theme.ts
 *
...
const THEME = {
  id: "theme_video_entrance",
  name: "video-entrance",
  label: "Video Entrance",
  description: "Single entrance video hero + Canva embed",

  // Layout selector — this is what makes InvitationView route to VideoEntrancePage
  layout: "video-entrance",
...
async function main() {
  console.log("Seeding video-entrance theme...");
  ...
  console.log("  4. Visit /<slug> — the video-entrance flow renders.");
}
